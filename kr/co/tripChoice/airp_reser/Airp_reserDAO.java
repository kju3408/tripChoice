...
public class Airp_reserDAO {
	...
	/*
	public int duplecateSeat(String ta_code, String tar_seat) {//String tar_seat는 원래 seatcol
        int cnt=0;
        try {
          con=dbopen.getConnection();          
          sql=new StringBuilder();
          sql.append(" SELECT COUNT(tar_code) as cnt");
          sql.append(" FROM airp_reser");
          sql.append(" WHERE ta_code=?");
          sql.append(" AND tar_seat=?");
          pstmt=con.prepareStatement(sql.toString());
          pstmt.setString(1, ta_code);
          pstmt.setString(2, tar_seat);
          rs=pstmt.executeQuery();
          if(rs.next()) {
              cnt=rs.getInt("cnt");
          }//if end
        }catch (Exception e) {
            System.out.println("예약된 좌석 확인 실패 : " + e);
        }finally {
