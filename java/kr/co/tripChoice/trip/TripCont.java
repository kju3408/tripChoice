...
public class TripCont {
	...
	@RequestMapping("/trip/list.do")
	public ModelAndView list(int trip_no) {
		ModelAndView mav = new ModelAndView();
		mav.setViewName("trip/korealist");
	...
	public ModelAndView koreaForm() {
		ModelAndView mav = new ModelAndView();
		mav.setViewName("trip/koreaForm");
		mav.addObject("root", Utility.getRoot());
		return mav;
	}//tripForm()end
	...
	public void createProc(@ModelAttribute TripDTO dto 
		...
		if(cnt==0) {
			...
			mav.addObject("link2", "<input type='button' value='목록' onclick='location.href=\"./create.do?trip_code=" +dto.getTrip_code() + "\"'>");
		}else {
			mav.addObject("msg1"  , "<p>여행상품 등록 성공! </p>");
			mav.addObject("img"   , "<img src='./images/heart.png'>");
			mav.addObject("link1" , "<input type='button' value='목록'  onclick='location.href=\"./create.do?trip_code=" +dto.getTrip_code() + "\"'>");
		}//if end 
	}// createProc() end 
		
	  @RequestMapping(value="/trip_korea/update.do", method=RequestMethod.GET)
	    public ModelAndView updateForm(TripDTO dto) {
	        ModelAndView mav = new ModelAndView();
	        mav.setViewName("trip_korea/updateForm");
	        mav.addObject("root", Utility.getRoot());
	        return mav;
	    }//updateForm() end
	   
	    @RequestMapping(value="/trip_korea/update.do", method=RequestMethod.POST)
	    public ModelAndView updateProc(TripDTO dto) {
	        ModelAndView mav = new ModelAndView();
	        mav.setViewName("trip_korea/msgView");
